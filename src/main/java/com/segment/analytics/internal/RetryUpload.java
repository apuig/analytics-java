...
public class RetryUpload implements Closeable, Runnable {
    ...
    public RetryUpload(
        ...
        final ScheduledFuture<?> unused = this.executor.scheduleWithFixedDelay(
    ...
    public void run() {
        ...
        final Queue<FileEntry> orderedFiles = storage.listFilesWithRetryAfterNow(1_000);
        while (!orderedFiles.isEmpty()) {
            final FileEntry entry = orderedFiles.poll();
            final Path tmpFile = storage.tryMoveToTmp(entry.file);
            if (tmpFile != null) {
                // NOTE: may use the calling thread (networkExecutor config)
